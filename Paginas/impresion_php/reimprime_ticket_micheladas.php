...
//if(!empty($_POST['user_id']) && !empty($_POST['tipo'])){
	...
	//generamos la consulta
									...
									$sql = "SELECT divs.Descripcion as divicion,cls.Descripcion as Divicion_tipo,pro.Descripcion,(p.Precio * count(*)) as Precio,count(*) as cantidad,
										        p.No_mesa,Nombre_Clientes, Folio FROM cl_productos as pro
										        ...
										        WHERE p.No_Mesa={$_POST['mesa']} and (p.Atendido=1 or p.Atendido=0) and p.Cuenta_individual={$_POST['tipo_cuenta']}
										              and p.Cuenta_cerrada=1 and p.Producto<>203 and p.Eliminados=0 and p.Folio={$_POST['folio']}
...
$consultas_="SELECT max(Folio) as Folio FROM `pedidos` WHERE Cuenta_cerrada=1 and Folio<>0
and No_Mesa={$_POST['mesa']} and Cuenta_individual={$_POST['tipo_cuenta']}"; // tiket max en genearl
//mysqli_query($db, $consultas_);
if(!$result2 = mysqli_query($db, $consultas_)) die();
while($row = mysqli_fetch_array($result2))
{
	$Folio = $row["Folio"];
}
 termina imprime original micheladas*/
/*$productos = array(
...
$printer->text("Sucursal Aldama" . "\n");
$printer->text("Lugar ". $mesa_num ."  Cliente: ".$nombre_cliente."        Ticket: ".$folio."\n");
