...
//var_dump($fecha_ini.' '.$fecha_finn);
  //$nom_cliente="'".$_POST['cliente']."'";
//generamos la consulta
//$sql = "SELECT No_Mesa,Descripcion,Ocupacion FROM `mesas` WHERE Zona={$_POST['zona']}";
/*$sql = "SELECT count(pro.Descripcion) as conteo,cls.Descripcion as Divicion,pro.Descripcion,sum(p.Precio) as tot FROM cl_productos as pro
        ...
        WHERE  (p.Atendido=0 or p.Atendido=1)
              and p.Cuenta_cerrada=1 and p.Producto<>203 and p.Eliminados=0 and p.Division=$tipo
              and Date(p.Fecha_Pedido) BETWEEN date($fecha_ini) and date($fecha_finn)
              GROUP by cls.Descripcion,pro.Descripcion,p.Precio
            union all
             SELECT count(p.Descrp_Nvo_Ingred),cls.Descripcion as Divicion,p.Descrp_Nvo_Ingred,sum(p.Precio_Nvo_Ingred) FROM cl_productos as pro
        left join pedidos as p on p.Producto= pro.ID_Producto
        left join cl_sub_diviciones as cls on cls.ID_Sub_Divicion=p.Sub_Divicion
        WHERE  (p.Atendido=0 or p.Atendido=1)
